...
如果设置了 `$OMNI_CONFIG`，OmniStream 会固定使用该路径，即使文件不存在也不会
回退。未设置时，按以下顺序使用第一个存在的配置文件：

1. `$XDG_CONFIG_HOME/omni-stream/config.toml`
2. `directories::ProjectDirs` 平台默认（macOS：`~/Library/Application Support/omni-stream/`；Linux：`~/.config/omni-stream/`）
3. `./config.toml`（当前目录）
...
> **注意**：`/raw` 文件挂载依赖 Bearer header 鉴权。浏览器导航和自动加载的子资源
> 无法附加该 header；程序发起的 fetch / XHR 只有显式设置 `Authorization` 才能附加。
> 因此浏览器直接打开的 `/raw` 仪表盘在 `public_read = false`（全锁定）模式下实际
> 不可用。默认的 `public_read = true` 模式下 `/raw` 正常工作。
