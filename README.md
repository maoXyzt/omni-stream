...
- `GET /raw/{storage}/{*path}` —— 可导航文件挂载：inline 提供文件（HTML 直接在浏览器渲染），目录路径或加 `?ls` 返回 JSON 列表，支持 copyparty 风格自包含 dashboard
...
# force_path_style = true  # 默认；部分 AOSS/OSS 虚拟主机网关设 false
```
