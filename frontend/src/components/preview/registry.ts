...
// File visual = icon + Tailwind text-color class. Groups are broader than
// PREVIEW_TYPES so the file list can show distinct icons for things that
// aren't (yet) previewable: audio, archives, spreadsheets, PDFs, etc.
...
for (const g of VISUAL_GROUPS) {
  for (const ext of g.exts) {
    EXT_TO_VISUAL.set(ext, { Icon: g.Icon, color: g.color })
  }
}
