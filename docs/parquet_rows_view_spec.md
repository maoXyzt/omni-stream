...
* **`storage` 覆盖**：image/video/audio/link/text 可选配置 `storage` storage name；省略时使用当前浏览的 parquet/jsonl 所属 storage。配置后只切换请求使用的 storage，`src` 相对路径仍以当前源数据文件目录为基准。名称不在 storage roster 中时渲染为错误，不发起请求。
